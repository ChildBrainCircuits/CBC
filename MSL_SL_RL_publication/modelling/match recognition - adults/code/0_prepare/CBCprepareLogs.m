...
if isempty(filenames)
    ...
    for i = 1:length(filenames)
        ...
        elseif contains(filename, 'AV')
            newlogTV.secondStim = logfile_trials.trials_runs_audio_stim; % trials_runs_auditory_stim for children
            newlogTV.mod2Type = repmat('aud', height(newlogTV),1);
        end
        
        newlogTV.frequency = logfile_trials.trials_runs_presentationFrequency;
        ...
        newlogTV.rewardAccurate = logfile_trials.trials_runs_rewardAccurate;
        
        newlogTV.stimPair = string(newlogTV.visStimLeft) + string(newlogTV.mod2Stim);
        newlogTV.match = logfile_trials.trials_runs_correct_comb;
        ...
        if ismember('yesKey', logfile_trials.Properties.VariableNames) %checks whether yesKey is logged
            ...
            idx = find(logfile_trials.trials_runs_rewardAccurate == 1 & logfile_trials.trials_runs_correct_comb == 1 & ...
                ~isnan(logfile_trials.response_runs_rt), 1); % finds first trial with correct combination and accurate reward
            if logfile_trials.trials_runs_correct_answer(idx) == 1
                yesKey = logfile_trials.response_runs_keys{idx};
            elseif logfile_trials.trials_runs_correct_answer(idx) == 0 && logfile_trials.response_runs_keys{idx} == 'z'
                yesKey = 'b';
            elseif logfile_trials.trials_runs_correct_answer(idx) == 0 && logfile_trials.response_runs_keys{idx} == 'b'
                yesKey = 'z';
            end            
        end
        ...
        newlogTV.session = repelem(order(i), height(newlogTV))';
