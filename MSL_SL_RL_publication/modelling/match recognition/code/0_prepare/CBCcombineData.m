function [allD, D, P, R, rpath] = CBCcombineData(OPTIONS, whichIDs)
...
%P is a summary of allD with one mean row (over all trials per task) 
% for each participant
if ismember('task', allD.Properties.VariableNames)
    allDnew=allD;
allDnew(:,[3,4]) = [];
P = varfun(@nanmean, allDnew, 'GroupingVariables', {'task', 'ID'});
...
% trial
D = varfun(@nanmean, allDnew, 'GroupingVariables', {'task', 'trial'});
