...
dataPath = fullfile('.', 'analyses', 'firstlevel', '1st_02_glm1_SPE_RPE_children_ICA', filesep);

% Path to store second-level analysis results
stats = fullfile('.', 'analyses', 'secondlevel', 'twosample_t-tests', '1st_02_glm2_SPE_RPE_discChoice_vs_matchRecog', filesep);

path_mask='';

%% Define subjects
% Read subject demographic data from CSV file
sublist = readtable(fullfile('.', 'data', 'outputs', 'finalSubjects.csv'), ...
    'VariableNamingRule','preserve', 'Delimiter',',');
sublist = sublist(~ismember(sublist.ID, 'CBC_1089'),:);
...
% match recognition group
sub1 = unique(sublist.ID(sublist.version == "v1"));
sub1 = sub1(~ismember(sub1, 'CBC_1089'));
% discriminative choice group
sub2 = unique(sublist.ID(sublist.version == "v3"));
...
for j=1:nCons
    ...
    % both task versions combined
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.name = [ConFolder{j} ' v1 + v3'];
    ...
    % match recog vs. disc choice
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.name = [ConFolder{j} ' v1 vs v3' ];
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.weights = [1 -1];
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.sessrep = 'none';
    c = c + 1;

    % disc choice vs. match recog
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.name = [ConFolder{j} ' v3 vs v1'];
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.weights = [-1 1];
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.sessrep = 'none';
    c = c + 1;

    % match recog
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.name = [ConFolder{j} ' v1'];
    ...
    % disc choice
    matlabbatch{3}.spm.stats.con.consess{c}.tcon.name = [ConFolder{j} ' v3'];
