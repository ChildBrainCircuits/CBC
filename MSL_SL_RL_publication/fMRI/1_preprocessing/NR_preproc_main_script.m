...
subsP2 = readtable(fullfile(basePath, 'scripts', 'statistical analyses', 'paper2', 'output', 'finalSubjects.csv'));

% adults ------------------------------------------------------------------
subsP2A = readtable(fullfile(basePath, 'scripts', 'statistical analyses', 'paper2', 'output', 'finalSampleAdults.csv'));

% Extract unique subject IDs from Paper 1, excluding 'NA' entries
subsAll = unique(subsP2.ID);
% subsAll = unique(subsP2A.ID);
...
% overview_subs(backupPath, savePath, 'adults')
...
% copy_files_for_preproc(subjects, backupPath, localPreproPath, 'adults')
...
% anatTemplate = fullfile(basePath, 'material', 'structural template', 'TPM.nii');
...
% copy_files_for_preproc(subjects, localPreproPath, preproPath, 'adults')
...
% logfilesB0(subjects, preproPath, 'adults');
