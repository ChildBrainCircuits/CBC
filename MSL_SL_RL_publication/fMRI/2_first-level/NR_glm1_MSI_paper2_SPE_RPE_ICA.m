...
firstlevelPath = '1st_02_glm1_SPE_RPE_children_ICA/'; % Folder for first-level GLM analysis
...
% discrimination choice task
sublistv3 = readtable([outputPath, 'outputfMRIv3AVeTVolderShort.csv'], ...
    'VariableNamingRule','preserve', 'Delimiter',';');
sublistv3.ACC_4th = num2cell(sublistv3.ACC_4th);

% match recognition task
sublistv1 = readtable([outputPath, 'outputfMRIv1AVeTV.csv'],...
    'VariableNamingRule','preserve', 'Delimiter',';');

% combine both groups
sublist = [sublistv3; sublistv1];
...
logFiles = readtable([outputPath, 'MRdata_childrenMR.csv'], ...
    'Delimiter',';', 'VariableNamingRule','modify');

% check if all subjects are in the logfiles
if length(unique(logFiles.ID)) ~= 58
    warning('children missing in logFiles')
end

% read modeling output data
modelingData = readtable([outputPath, 'modellingOuputfMRI.csv'], ...
    'Delimiter',',', 'VariableNamingRule','modify');
...
if length(unique(modelingData.ID)) ~= 57
    warning('children missing in modelingData')
end

% read surprise output data
surpriseData =  readtable([outputPath, 'surpriseOuputfMRI.csv'], ...
    'Delimiter',',', 'VariableNamingRule','modify');
...
if length(unique(surpriseData.ID)) ~= 58
...
% exclude subject
subject(ismember(subject,'CBC_1089')) = [];
...
for i=1:length(subject)
    ...
    for sess = 1:length(currList.sess)
        ...
        % Determine stimulus onsets based on task version and modality
        if contains(logfilename, "v3")
        ...
        elseif contains(logfilename, "v1")
            if contains(logfilename, "TV")
                speOnset = logfile_table.('tactile_4_started')(idxStim)-first_scan;
            elseif contains(logfilename, "AV")
                speOnset = logfile_table.('sound_4_started')(idxStim)-first_scan;
            end
        end
        ...
        if ~isempty(idxFBneut)
            matlabbatch{1}.spm.stats.fmri_spec.sess(j).cond(3).onset = logfile_table.('feedback_6_started')(idxFBneut)-first_scan;
        else
        ...
        if ~isempty(idxRT)
            matlabbatch{1}.spm.stats.fmri_spec.sess(j).cond(4).onset = logfile_table.('feedback_6_started')(idxRT)-first_scan;
        else
